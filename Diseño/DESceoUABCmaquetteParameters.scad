...
//openbeam standards
OBoutside=15;//the outside size of the beam
OBchannel=5.8;//the channel thickness; where the 4-40 nut slides
OBslotDepth=1.65;//the depth of the screwhead hole, is also used for a just sunk in groove.
OBstep=(OBoutside-OBchannel)/2;//this is the step size on either side
OBslot=3.2;//this is the slot for the screw head
OBcubeBlock=OBchannel+OBstep;//the y side of the cube. The x is OBchannel
OBchannelDepth=3.75;//the depth of the channel and point
OBnutHeight=6.2;//the height that a nut in the channel takes up
...
RailSink=Ph/2+OBoutside/2-OBstep-OBchannel+2*wiggle;
RailCenter=OBoutside/2	;//2* the distance from the center where the rails start
OBlength=r-RailCenter;//the length of the beam
