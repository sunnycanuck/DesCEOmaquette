...
// Pulley properties
shaftDiameter = 5.1; //8 the shaft at the center, will be subtracted from the pulley. Better be too small than too wide.
hubDiameter = 10; //13 if the hub or timing pulley is big enough to fit a nut, this will be embedded.
hubHeight = 8; //10 the hub is the thick cylinder connected to the pulley to allow a set screw to go through or as a collar for a nut.
...
// Belt properties:
pitch = 2.07; //5 distance between the teeth
beltWidth = 3; //4 the width/height of the belt. The (vertical) size of the pulley is adapted to this.
beltThickness = .6; //.65 thickness of the part excluding the notch depth!
notchDepth = 1.5; //1.8 make it slightly bigger than actual, there's an outward curvature in the inner solid part of the pulley
toothWidth = .6; // Teeth of the PULLEY, that is.
...
module timingPulley(
	...
	module timingGear(pitch,beltWidth,beltThickness,numTeeth,notchDepth,flanges)
	{
		...
		union()
		{
			...
			{
				//top flange
				translate([0,0,0]) cylinder(h = 1, r1=outerRadius,r2=outerRadius+2);
				...
				//bottom flange
				translate([0,0,-toothHeight-0.5]) cylinder(h = 1, r2=outerRadius,r1=outerRadius+2);
